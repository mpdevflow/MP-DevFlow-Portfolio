        ...
        <div className="flex flex-col md:flex-row justify-between items-end mb-16 gap-6">
          <div>
            ...
            <p className="text-slate-400 max-w-lg">
              Client work and in-house studio products. Every project ships with the same
              senior-level care.
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {PROJECTS.map((project, index) => (
            ...
            >
              {/* Card Preview Area */}
              <div
                className={`h-48 w-full bg-gradient-to-br ${project.accentColor ?? 'from-slate-900 to-slate-800'} flex items-center justify-center relative overflow-hidden`}
              >
                {/* Studio / Client badge — top-left */}
                <div className="absolute top-4 left-4 z-20">
                  {project.isStudioProject ? (
                    <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-violet-500/20 border border-violet-400/30 text-violet-300 text-[10px] font-bold uppercase tracking-wider backdrop-blur-sm">
                      <span>🏗️</span> Studio Project
                    </span>
                  ) : (
                    <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-cyan-500/20 border border-cyan-400/30 text-cyan-300 text-[10px] font-bold uppercase tracking-wider backdrop-blur-sm">
                      Client Work
                    </span>
                  )}
                </div>
              ...
              <div className="p-8 flex-1 flex flex-col">
                <div
                  className={`text-xs font-semibold uppercase tracking-wider mb-2 ${project.isStudioProject ? 'text-violet-400' : 'text-cyan-400'}`}
                >
                  {project.category}
                </div>
